...
from src.services.synthetic_data_service import data_service as es_service  # 🔄 swap import when ELK is ready
...
def analyze_transactions(request: InferenceRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Failed to fetch from Elasticsearch: {e}")
    ...
    patterns = pattern_recognizer.analyze(transactions, account_id=request.account_id or "all")
